...
public class Shop_Deck {
	...
	void basket() {
		...
		if(list.isEmpty()) {
			...
			System.out.printf("상품리스트 개수 : %d 개\n",list.size());
			for(int i=0; i<list.size(); i++) {
				System.out.printf("제품 : %s \t",list.get(i).getName());
	...
	void login() {
		while(true) {
			...
			if(id.equals("admin") && pw.equals("1111")) {
				// db 접근
				user1 = new Buyer("admin","1111","관리자","서울시 압구정동",100000000,1000000);
